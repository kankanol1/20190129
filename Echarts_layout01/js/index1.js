...
(function(){
    ...
    function methodRandom(nodesSection){
        ...
        for(let i=0;i<nodes.length;i++){
            nodes[i].position_X = c.width/2-c.width/6 + R*Math.cos(i/nodes.length*2*Math.PI);
            nodes[i].position_Y = c.height/2-c.width/6+dis + R*Math.sin(i/nodes.length*2*Math.PI);
        }
    ...
    function createElementPay(link,nodes,x,y,centerNodes){
        // ct.clearRect(0,0,c.width,c.height);
        // drawing(nodes,links,centerNodes);//绘制图形
        drawLine(checkIndex(link.startNode,nodes),checkIndex(link.endNode,nodes),link.count,"#fff",.5);
    ...
    //判断节点是否为节点
    function checkPointNode(n,nodes){
        let flag = 1;
        for(let i=0;i<nodes.length;i++){
            if(n == nodes[i].id){
                flag = 0;
