...
(function(){
    ...
    for(let i=0;i<centerNodes.length;i++){
        nodes[checkIndexId(centerNodes[i],nodes)].position_X = c.width/3 + R*Math.sin(i/centerNodes.length*2*Math.PI);
        nodes[checkIndexId(centerNodes[i],nodes)].position_Y = c.height/3 + R*Math.cos(i/centerNodes.length*2*Math.PI);
    }
    ...
    for(let i=0;i<centerNodes.length;i++){
        ...
        let alpha = Math.atan((c.height/3-nodes[flag].position_Y)/(c.width/3-nodes[flag].position_X));
        let n = 0;//标记
        let outL = 3;//放大系数
        for(let j=0;j<nodes.length;j++){
    ...
    for(let i=0;i<splitNodes.length;i++){
        for(let j=1;j<splitNodes[i].length;j++){
            nodes[splitNodes[i][j]].position_X = nodes[splitNodes[i][0]].position_X - 0.8*c.width/6*Math.cos(j*Math.PI*2/(splitNodes[i].length-1));
            nodes[splitNodes[i][j]].position_Y = nodes[splitNodes[i][0]].position_Y - 0.8*c.width/6*Math.sin(j*Math.PI*2/(splitNodes[i].length-1));
        }
    }
/*    for(let j=1;j<splitNodes[0].length;j++){
        nodes[splitNodes[0][j]].position_X = c.width/6 + 2*c.width/6*(1-Math.random());
        nodes[splitNodes[0][j]].position_Y = c.height/2+ 2*c.width/6*(1-Math.random());
    }*/
        ...
        for(let i=0;i<nodes.length;i++){
            for(let j=0;j<links.length;j++){
                ...
                if(nodes[i].id.toString() == idF){
                    ...
                    if(flag_line>-1){
                        drawLine(nodes[i],nodes[flag_line],linkPay,"#000");
                    }
                }
            }
        }
