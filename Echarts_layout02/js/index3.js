...
(function () {
    ...
    for (let i = 0; i < centerNodes.length; i++) {
        nodes[checkIndexId(centerNodes[i], nodes)].position_X = ratioW + R * Math.sin(i / centerNodes.length * 2 * Math.PI);
        nodes[checkIndexId(centerNodes[i], nodes)].position_Y = ratioH + R * Math.cos(i / centerNodes.length * 2 * Math.PI);
    }
    ...
    for (let i = 0; i < centerNodes.length; i++) {
        ...
        let alpha = Math.atan((ratioW - nodes[flag].position_Y) / (ratioH - nodes[flag].position_X));
        let n = 0;//标记
        let outL = 3;//放大系数
        for (let j = 0; j < nodes.length; j++) {
            if (matrix[flag][j] == 1 && nodes[j].degree == 1) {
                ...
                if (nodes[flag].position_X - c.width / (ratio / 0.5) > 0 && nodes[flag].position_Y - ratioH > 0) {
                    if (n % 2) {
                ...
                if (nodes[flag].position_X - c.width / (ratio / 0.5) > 0 && nodes[flag].position_Y - ratioH <= 0) {
                    if (n % 2) {
                        nodes[j].position_X = c.width / (ratio / 0.5) + scaleC * Math.cos(alpha - chang);
                        nodes[j].position_Y = ratioH + scaleC * Math.sin(alpha - chang);
                    } else {
                        nodes[j].position_X = c.width / (ratio / 0.5) + scaleC * Math.cos(alpha + chang);
                        nodes[j].position_Y = ratioH + scaleC * Math.sin(alpha + chang);
                ...
                if (nodes[flag].position_X - c.width / (ratio / 0.5) <= 0 && nodes[flag].position_Y - ratioH <= 0) {
                    if (n % 2) {
                        nodes[j].position_X = c.width / (ratio / 0.5) + scaleC * Math.cos(alpha - chang + Math.PI);
    ...
    function drawLine(nodeO, nodeT, pay, color, lineWidth) {
        ...
        if (nodeO.position_X > nodeT.position_X && nodeO.position_Y > nodeT.position_Y) {
            ...
            ct.translate((nodeO.position_X + nodeT.position_X) / 2, (nodeO.position_Y + nodeT.position_Y) / 2);//移位
            ct.rotate(-(alpha - Math.PI / 2));
            ct.fillText("支付(" + pay + ")", 0, 0);
            ct.rotate((alpha - Math.PI / 2));
            ct.translate(-(nodeO.position_X + nodeT.position_X) / 2, -(nodeO.position_Y + nodeT.position_Y) / 2);//复位
        }
        if (nodeO.position_X > nodeT.position_X && nodeO.position_Y <= nodeT.position_Y) {
            ct.moveTo(nodeO.position_X - Math.cos(beta) * (radius + 5), nodeO.position_Y + Math.sin(beta) * (radius + 5));
            ct.lineTo(mdx + Math.sin(alpha) * textL, mdy - Math.cos(alpha) * textL);
        ...
        for (let i = 0; i < nodes.length; i++) {
            for (let j = 0; j < links.length; j++) {
                ...
                if (nodes[i].id.toString() == idF) {
                    ...
                    if (flag_line > -1) {
                        drawLine(nodes[i], nodes[flag_line], linkPay, "#666", 0.05);
                    }
                }
            }
        }
